...
public class UserService extends ServiceImpl<UserMapper, User> implements InitializingBean {
  ...
  @PostConstruct
  public void init() {
    QuickConfig idQc = QuickConfig.newBuilder(":user:cache:id:").cacheType(CacheType.BOTH).expire(Duration.ofHours(2))
  ...
  public User wxLogin(String code, String nickName, String avatarUrl, Integer gender, String city, String province,
    ...
    if (user == null) {
      ...
      // 加入流水
      userOperateStreamService.insertStream(user, UserOperateTypeEnum.REGISTER);
    } else {
      // 用户存在，更新用户信息
  ...
  @Transactional(rollbackFor = Exception.class)
  public UserOperatorResponse updateProfile(Long userId, String nickName, String phone, String avatarUrl) {
    ...
    if (updateById(user)) {
      // 加入流水
      userOperateStreamService.insertStream(user, UserOperateTypeEnum.MODIFY);
